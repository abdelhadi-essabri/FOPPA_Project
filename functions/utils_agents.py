...
def clean_zipcode_based_on_city(df):
    ...
    mode_zipcode_by_city = df.dropna().groupby('city')['cleaned_zipcode'].agg(lambda x: x.mode()[0] if not x.mode().empty else np.nan)
    ...
    df['cleaned_zipcode'] = df.apply(
        lambda row: mode_zipcode_by_city.get(row['city'], global_mode_zipcode) if pd.isna(row['cleaned_zipcode']) else row['cleaned_zipcode'],
        axis=1
    )
